...
static CGFloat const DEFAULT_MASK_ALPHA = 0.75;//阴影
static bool  square = YES; //是否是正方形
...
-(id)initWithImage:(UIImage*)image{
    ...
    return [self initWithImage:image minSize:CGSizeMake(100, 100) isSquare:YES];
}

-(id)initWithImage:(UIImage *)image minSize:(CGSize)minSize isSquare:(bool) isSquare{

    self = [super init];
    if (self) {
        self.image = [image fixOrientation];
        self.view.backgroundColor = [UIColor blackColor];
        IMAGE_MIN_WIDTH = minSize.width;
        IMAGE_MIN_HEIGHT = minSize.height;
        square = isSquare;
    }
    return self;
}
